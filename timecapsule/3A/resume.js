...
const DATA = {
  ...
  ko: {
    ...
    profile: {
      ...
      summary:
        "비개발자도 바로 사용할 수 있는 사내 AI 업무도구를 ‘운영 가능한 형태’로 설계·구현·배포해왔습니다. 로그인/RBAC, 감사 로그, 버전 관리·배포, 민감정보 유출 위험 키워드 감지(알림)까지 포함한 거버넌스 기능을 구축한 경험이 있습니다."
    },
    contact: {
      siteLabel: "웹",
      site: "git.io/JXa1p",
    ...
    experience: [
      {
        ...
        bullets: [
          "약 50명 규모 조직을 대상으로 사내 LLM 업무 보조 도구(베타) 개발을 주도: Windows 데스크톱(PySide6) + FastAPI 서버(서버실 상시 구동).",
          "DB 기반 거버넌스 구현: 로그인/권한(RBAC), 버전 관리·배포, 사용/프롬프트 감사 로그(사용자·IP/MAC·앱 버전)로 컴플라이언스 및 디버깅 지원.",
          "프롬프트 마켓(약 10개 템플릿) 구축: 사용자 기여(작성자 표시) + 관리자 검수/삭제. 민감정보 유출 위험 A/B/C 키워드 임계치 초과 시 관리자 알림(팝업) 제공.",
...
function render(){
  ...
  // ensure dropdown stays visible and correct
...
$("langSel").addEventListener("change", (e)=>{
  // remember paper for current lang
...
// keep @page correct at print time
...
async function safePrint(){
  ...
  // wait two frames so layout is fully committed (helps Safari/Chrome)
